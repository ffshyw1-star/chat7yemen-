...
export const ToolsView: React.FC<{ showToast: (msg: string) => void }> = ({ showToast }) => {
  ...
  const [isUpdatingBackups, setIsUpdatingBackups] = useState(false);

  const handleSaveLimits = async () => {
    if (isSavingLimits || isUpdatingBackups) return;
    ...
    try {
      ...
      showToast('⚠️ حدث خطأ أثناء حفظ الإعدادات في قاعدة البيانات');
    } finally {
  ...
  const handleCreateBackup = async () => {
    ...
    if (isSavingLimits || isUpdatingBackups) return;
    setIsUpdatingBackups(true);
    try {
      await updateSiteSettings({ backups: updatedBackups });
      ...
      showToast('⚠️ حدث خطأ أثناء حفظ النسخة الاحتياطية في قاعدة البيانات');
    } finally {
      setIsUpdatingBackups(false);
    }
  };
    ...
    reader.onload = async (ev) => {
      try {
        ...
        if (parsed.siteSettings) {
          setIsUpdatingBackups(true);
          await updateSiteSettings(parsed.siteSettings);
        }
  ...
  const handleDeleteBackupItem = async (id: string) => {
    if (isSavingLimits || isUpdatingBackups) return;
    setIsUpdatingBackups(true);
    try {
      const currentBackups = siteSettings?.backups || [];
      ...
      showToast('⚠️ حدث خطأ أثناء حذف النسخة في قاعدة البيانات');
    } finally {
      setIsUpdatingBackups(false);
    }
  };
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 sm:p-5 shadow-2xs space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          ...
          <button
            ...
            disabled={isSavingLimits || isUpdatingBackups}
            className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white font-black text-xs rounded-xl cursor-pointer shadow-xs transition-colors flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isSavingLimits ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5" />}
            <span>{isSavingLimits ? 'جاري الحفظ...' : 'حفظ الإعدادات 💾'}</span>
          </button>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={isSavingLimits || isUpdatingBackups}
              className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-lg cursor-pointer transition-colors flex items-center gap-1.5 shadow-2xs disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isUpdatingBackups ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5" />}
              <span>{isUpdatingBackups ? 'جارٍ الحفظ...' : 'أخذ نسخة سحابية 💾'}</span>
            </button>

            <button
              onClick={handleDownloadBackup}
              disabled={isSavingLimits || isUpdatingBackups}
              className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-lg cursor-pointer transition-colors flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              ...
              <div key={item.id} className="flex items-center justify-between p-3 bg-slate-50/50 hover:bg-slate-50 transition-colors">
                ...
                <div className="flex items-center gap-2">
                      ...
                      if (window.confirm('هل تريد استعادة هذه النسخة الاحتياطية وتطبيقها على الموقع؟')) {
                        try {
                          ...
                          if (stored) {
                            ...
                            if (parsed.data?.siteSettings) await updateSiteSettings(parsed.data.siteSettings);
                          ...
                          console.error('Failed to restore backup:', err);
                          showToast('⚠️ فشل تطبيق النسخة الاحتياطية');
                        } finally {
                          setIsUpdatingBackups(false);
                        }
                      }
                    }}
                    className="px-2.5 py-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 text-xs font-bold rounded-lg cursor-pointer flex items-center gap-1 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                  ...
                  <button
                    ...
                    className="p-1.5 text-rose-500 hover:bg-rose-50 rounded-lg cursor-pointer transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    title="حذف النسخة"
                  >
